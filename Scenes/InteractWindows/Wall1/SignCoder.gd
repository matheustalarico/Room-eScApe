...
@onready var password_screen: HBoxContainer = $PasswordScreen
...
func _ready() -> void:
	SignalHub.on_digit_button_pressed.connect(on_digit_button_pressed)
	write_on_screen()


func write_on_screen() -> void:
	for digit in password_screen.get_children():
		if digit is Digit:
...
func on_digit_button_pressed(digit_number: int) -> void:
	if current_digit <= 7:
		...
		if current_digit == 8:
			SignalHub.emit_on_sign_password_complete(digits)
			timer.start()
